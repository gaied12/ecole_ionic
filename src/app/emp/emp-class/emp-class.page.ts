...
export class EmpClassPage implements OnInit {
  ...
  constructor(private service:StudentService,private clasSer:ClassService,private fb:FormBuilder,private alert:AlertController,private router:Router) { 
    this.fTab=this.fb.group({ classId : ['', Validators.compose([
      Validators.required,
     ]),
...
console.log(this.class)
  ...
  async add(x:any) {
    const alert = await this.alert.create({
      ...
      subHeader: '',
      message: 'vous voulez Ajouter un emploi au classe '+ this.ClasName(x),
      buttons: [
